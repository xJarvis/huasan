...
/**
* 数据库TIMESTAMP类型时间转换为DATE类型时间(todo 废弃，请勿使用)
*/
func FormatDBDate(timeStr string) (string,error) {
	t, err := time.Parse(TIME_DB_TIMESTAMP_FORMAT, timeStr)
	if err != nil {
		return "",err
	}
	return t.Format(DATE_BASE_FORMAT),nil
}

/**
* 数据库TIMESTAMP类型时间转换为DATETIME类型时间(todo 废弃，请勿使用)
*/
func FormatDBDateTime(timeStr string) (string,error) {
	t, err := time.Parse(TIME_DB_TIMESTAMP_FORMAT,timeStr)
	if err != nil {
		return "",err
	}
	return t.Format(TIME_BASE_FORMAT),nil
}
