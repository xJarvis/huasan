...
import (
	"fmt"
	client2 "github.com/xjarvis/huashan/nosql/influxdb/client"
	"math/rand"
	"os"
	"time"
)
...
func ExampleClient() {
	...
	_, err := client2.NewHTTPClient(client2.HTTPConfig{
...
func ExampleClient_uDP() {
	// Make client
	config := client2.UDPConfig{Addr: "localhost:8089"}
	c, err := client2.NewUDPClient(config)
	if err != nil {
	...
	bp, _ := client2.NewBatchPoints(client2.BatchPointsConfig{
		Precision: "s",
	})
	...
	pt, err := client2.NewPoint("cpu_usage", tags, fields, time.Now())
	if err != nil {
		fmt.Println("Error: ", err.Error())
	}
	bp.AddPoint(pt)
...
func ExamplePoint_withoutTime() {
	...
	pt, err := client2.NewPoint("cpu_usage", tags, fields)
	if err == nil {
...
func ExampleClient_query() {
	...
	q := client2.NewQuery("SELECT count(value) FROM shapes", "square_holes", "ns")
...
func ExampleClient_createDatabase() {
	...
	q := client2.NewQuery("CREATE DATABASE telegraf", "", "")
	if response, err := c.Query(q); err == nil && response.Error() == nil {
		fmt.Println(response.Results)
	}
}
