...
import (
	"bytes"
	"fmt"
	models2 "github.com/xjarvis/huashan/nosql/influxdb/models"
	"io"
	"math"
	"math/rand"
	"reflect"
...
var (
	tags   = models2.NewTags(map[string]string{"foo": "bar", "apple": "orange", "host": "serverA", "region": "uswest"})
	fields = models2.Fields{
...
func TestMarshalFields(t *testing.T) {
	for _, tt := range []struct {
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			if have, want := models2.Fields(fields).MarshalBinary(), []byte(tt.exp); !bytes.Equal(have, want) {
...
func TestTags_HashKey(t *testing.T) {
	tags = models2.NewTags(map[string]string{"A FOO": "bar", "APPLE": "orange", "host": "serverA", "region": "uswest"})
	got := tags.HashKey()
...
func TestPoint_Tags(t *testing.T) {
	examples := []struct {
		...
		{`cpu value=1`, models2.Tags{}},
		{"cpu,tag0=v0 value=1", models2.NewTags(map[string]string{"tag0": "v0"})},
		{"cpu,tag0=v0,tag1=v0 value=1", models2.NewTags(map[string]string{"tag0": "v0", "tag1": "v0"})},
		{`cpu,tag0=v\ 0 value=1`, models2.NewTags(map[string]string{"tag0": "v 0"})},
		{`cpu,tag0=v\ 0\ 1,tag1=v2 value=1`, models2.NewTags(map[string]string{"tag0": "v 0 1", "tag1": "v2"})},
		{`cpu,tag0=\, value=1`, models2.NewTags(map[string]string{"tag0": ","})},
		{`cpu,ta\ g0=\, value=1`, models2.NewTags(map[string]string{"ta g0": ","})},
		{`cpu,tag0=\,1 value=1`, models2.NewTags(map[string]string{"tag0": ",1"})},
		{`cpu,tag0=1\"\",t=k value=1`, models2.NewTags(map[string]string{"tag0": `1\"\"`, "t": "k"})},
	}

	for _, example := range examples {
		t.Run(example.Point, func(t *testing.T) {
			pts, err := models2.ParsePointsString(example.Point)
			if err != nil {
				t.Fatal(err)
			} else if len(pts) != 1 {
...
func TestPoint_StringSize(t *testing.T) {
	testPoint_cube(t, func(p models2.Point) {
		l := p.StringSize()
		s := p.String()
...
func testPoint_cube(t *testing.T, f func(p models2.Point)) {
	// heard of a table-driven test? let's make a cube-driven test...
	tagList := []models2.Tags{nil, {models2.NewTag([]byte("foo"), []byte("bar"))}, tags}
	fieldList := []models2.Fields{{"a": 42.0}, {"a": 42, "b": "things"}, fields}
	...
	for _, tagSet := range tagList {
		for _, fieldSet := range fieldList {
			for _, pointTime := range timeList {
				p, err := models2.NewPoint("test", tagSet, fieldSet, pointTime)
				if err != nil {
...
func TestTag_Clone(t *testing.T) {
	tag := models2.NewTag([]byte("key"), []byte("value"))
...
func TestTags_Clone(t *testing.T) {
	tags := models2.NewTags(map[string]string{"k1": "v1", "k2": "v2", "k3": "v3"})
...
func BenchmarkNewPoint(b *testing.B) {
	ts := time.Now()
	for i := 0; i < b.N; i++ {
		p, _ = models2.NewPoint("measurement", tags, fields, ts)
	}
}

func BenchmarkNewPointFromBinary(b *testing.B) {
	pts, err := models2.ParsePointsString("cpu value1=1.0,value2=1.0,value3=3.0,value4=4,value5=\"five\" 1000000000")
	if err != nil {
	...
	for i := 0; i < b.N; i++ {
		_, err := models2.NewPointFromBytes(bytes)
		if err != nil {
...
func BenchmarkParsePointNoTags5000(b *testing.B) {
	...
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		models2.ParsePoints([]byte(lines))
...
func BenchmarkParsePointWithPrecisionN(b *testing.B) {
	...
	for i := 0; i < b.N; i++ {
		models2.ParsePointsWithPrecision([]byte(line), defaultTime, "n")
		b.SetBytes(int64(len(line)))
	}
}
...
func BenchmarkParsePointWithPrecisionU(b *testing.B) {
	...
	defaultTime := time.Now().UTC()
	for i := 0; i < b.N; i++ {
		models2.ParsePointsWithPrecision([]byte(line), defaultTime, "u")
		b.SetBytes(int64(len(line)))
	}
}
...
func BenchmarkParsePointsTagsUnSorted2(b *testing.B) {
	...
	for i := 0; i < b.N; i++ {
		pt, _ := models2.ParsePoints([]byte(line))
		b.SetBytes(int64(len(line)))
		pt[0].Key()
	}
}
...
type TestPoint struct {
	RawFields models2.Fields
	RawTags   models2.Tags
	RawTime   time.Time
	models2.Point
}
...
func NewTestPoint(name string, tags models2.Tags, fields models2.Fields, time time.Time) TestPoint {
	return TestPoint{
		RawTags:   tags,
		RawFields: fields,
		RawTime:   time,
		Point:     models2.MustNewPoint(name, tags, fields, time),
	}
}

func test(t *testing.T, line string, point TestPoint) {
	pts, err := models2.ParsePointsWithPrecision([]byte(line), time.Unix(0, 0), "n")
	if err != nil {
...
func TestParsePointNoValue(t *testing.T) {
	pts, err := models2.ParsePointsString("")
	if err != nil {
...
func TestParsePointNoFields(t *testing.T) {
	...
	for i, example := range examples {
		_, err := models2.ParsePointsString(example)
		if err == nil {
...
func TestParsePointNoTimestamp(t *testing.T) {
	test(t, "cpu value=1", NewTestPoint("cpu", nil, models2.Fields{"value": 1.0}, time.Unix(0, 0)))
}
...
func TestParsePointMissingTagKey(t *testing.T) {
	...
	_, err := models2.ParsePointsString(`cpu,host=serverA,\ =us-east value=1i`)
	if err != nil {
...
func TestParsePointMissingFieldName(t *testing.T) {
	_, err := models2.ParsePointsString(`cpu,host=serverA,region=us-west =`)
	if err == nil {
		t.Errorf(`ParsePoints("%s") mismatch. got nil, exp error`, `cpu,host=serverA,region=us-west =`)
	}

	_, err = models2.ParsePointsString(`cpu,host=serverA,region=us-west =123i`)
	if err == nil {
		t.Errorf(`ParsePoints("%s") mismatch. got nil, exp error`, `cpu,host=serverA,region=us-west =123i`)
	}

	_, err = models2.ParsePointsString(`cpu,host=serverA,region=us-west a\ =123i`)
	if err != nil {
		t.Errorf(`ParsePoints("%s") mismatch. got nil, exp error`, `cpu,host=serverA,region=us-west a\ =123i`)
	}
	_, err = models2.ParsePointsString(`cpu,host=serverA,region=us-west value=123i,=456i`)
	if err == nil {
...
func TestParsePointBadNumber(t *testing.T) {
	for _, tt := range []string{
		...
		_, err := models2.ParsePointsString(tt)
		if err == nil {
...
func TestParsePointMaxInt64(t *testing.T) {
	// out of range
	_, err := models2.ParsePointsString(`cpu,host=serverA,region=us-west value=9223372036854775808i`)
	...
	// max int
	p, err := models2.ParsePointsString(`cpu,host=serverA,region=us-west value=9223372036854775807i`)
	if err != nil {
	...
	// leading zeros
	_, err = models2.ParsePointsString(`cpu,host=serverA,region=us-west value=0009223372036854775807i`)
	if err != nil {
...
func TestParsePointMinInt64(t *testing.T) {
	// out of range
	_, err := models2.ParsePointsString(`cpu,host=serverA,region=us-west value=-9223372036854775809i`)
	if err == nil {
		t.Errorf(`ParsePoints("%s") mismatch. got nil, exp error`, `cpu,host=serverA,region=us-west value=-9223372036854775809i`)
	}

	// min int
	p, err := models2.ParsePointsString(`cpu,host=serverA,region=us-west value=-9223372036854775808i`)
	if err != nil {
	...
	// leading zeros
	_, err = models2.ParsePointsString(`cpu,host=serverA,region=us-west value=-0009223372036854775808i`)
	if err != nil {
...
func TestParsePointMaxFloat64(t *testing.T) {
	// out of range
	_, err := models2.ParsePointsString(fmt.Sprintf(`cpu,host=serverA,region=us-west value=%s`, "1"+string(maxFloat64)))
	if err == nil {
		t.Errorf(`ParsePoints("%s") mismatch. got nil, exp error`, `cpu,host=serverA,region=us-west value=...`)
	}

	// max float
	p, err := models2.ParsePointsString(fmt.Sprintf(`cpu,host=serverA,region=us-west value=%s`, string(maxFloat64)))
	if err != nil {
	...
	// leading zeros
	_, err = models2.ParsePointsString(fmt.Sprintf(`cpu,host=serverA,region=us-west value=%s`, "0000"+string(maxFloat64)))
	if err != nil {
...
func TestParsePointMinFloat64(t *testing.T) {
	// out of range
	_, err := models2.ParsePointsString(fmt.Sprintf(`cpu,host=serverA,region=us-west value=%s`, "-1"+string(minFloat64)[1:]))
	if err == nil {
		t.Errorf(`ParsePoints("%s") mismatch. got nil, exp error`, `cpu,host=serverA,region=us-west value=...`)
	}

	// min float
	p, err := models2.ParsePointsString(fmt.Sprintf(`cpu,host=serverA,region=us-west value=%s`, string(minFloat64)))
	if err != nil {
	...
	// leading zeros
	_, err = models2.ParsePointsString(fmt.Sprintf(`cpu,host=serverA,region=us-west value=%s`, "-0000000"+string(minFloat64)[1:]))
	if err != nil {
		t.Errorf(`ParsePoints("%s") mismatch. got %v, exp nil`, `cpu,host=serverA,region=us-west value=...`, err)
	}
}
...
func TestParsePointMaxUint64(t *testing.T) {
	// out of range
	_, err := models2.ParsePointsString(`cpu,host=serverA,region=us-west value=18446744073709551616u`)
	...
	// max int
	p, err := models2.ParsePointsString(`cpu,host=serverA,region=us-west value=18446744073709551615u`)
	if err != nil {
	...
	// leading zeros
	_, err = models2.ParsePointsString(`cpu,host=serverA,region=us-west value=00018446744073709551615u`)
	if err != nil {
...
func TestParsePointMinUint64(t *testing.T) {
	// out of range
	_, err := models2.ParsePointsString(`cpu,host=serverA,region=us-west value=--1u`)
	if err == nil {
		t.Errorf(`ParsePoints("%s") mismatch. got nil, exp error`, `cpu,host=serverA,region=us-west value=-1u`)
	}

	// min int
	p, err := models2.ParsePointsString(`cpu,host=serverA,region=us-west value=0u`)
	if err != nil {
	...
	// leading zeros
	_, err = models2.ParsePointsString(`cpu,host=serverA,region=us-west value=0000u`)
	if err != nil {
...
func TestParsePointNumberNonNumeric(t *testing.T) {
	_, err := models2.ParsePointsString(`cpu,host=serverA,region=us-west value=.1a`)
	if err == nil {
...
func TestParsePointNegativeWrongPlace(t *testing.T) {
	_, err := models2.ParsePointsString(`cpu,host=serverA,region=us-west value=0.-1`)
	if err == nil {
...
func TestParsePointFloatMultipleDecimals(t *testing.T) {
	_, err := models2.ParsePointsString(`cpu,host=serverA,region=us-west value=1.1.1`)
	if err == nil {
...
func TestParsePointInteger(t *testing.T) {
	_, err := models2.ParsePointsString(`cpu,host=serverA,region=us-west value=1i`)
	if err != nil {
...
func TestParsePointNegativeFloat(t *testing.T) {
	_, err := models2.ParsePointsString(`cpu,host=serverA,region=us-west value=-1.0`)
	if err != nil {
...
func TestParsePointFloatNoLeadingDigit(t *testing.T) {
	_, err := models2.ParsePointsString(`cpu,host=serverA,region=us-west value=.1`)
	if err != nil {
		t.Errorf(`ParsePoints("%s") mismatch. got %v, exp nil`, `cpu,host=serverA,region=us-west value=-1.0`, err)
	}
}
...
func TestParsePointFloatScientific(t *testing.T) {
	_, err := models2.ParsePointsString(`cpu,host=serverA,region=us-west value=1.0e4`)
	if err != nil {
		t.Errorf(`ParsePoints("%s") mismatch. got %v, exp nil`, `cpu,host=serverA,region=us-west value=1.0e4`, err)
	}

	pts, err := models2.ParsePointsString(`cpu,host=serverA,region=us-west value=1e4`)
	if err != nil {
		t.Errorf(`ParsePoints("%s") mismatch. got %v, exp nil`, `cpu,host=serverA,region=us-west value=1.0e4`, err)
	}
...
func TestParsePointFloatScientificDecimal(t *testing.T) {
	_, err := models2.ParsePointsString(`cpu,host=serverA,region=us-west value=1.0e-4`)
	if err != nil {
...
func TestParsePointBooleanInvalid(t *testing.T) {
	_, err := models2.ParsePointsString(`cpu,host=serverA,region=us-west value=a`)
	if err == nil {
...
func TestParsePointScientificIntInvalid(t *testing.T) {
	_, err := models2.ParsePointsString(`cpu,host=serverA,region=us-west value=9ie10`)
	if err == nil {
		t.Errorf(`ParsePoints("%s") mismatch. got nil, exp error`, `cpu,host=serverA,region=us-west value=9ie10`)
	}

	_, err = models2.ParsePointsString(`cpu,host=serverA,region=us-west value=9e10i`)
	if err == nil {
...
`,
	...
	expPoint := NewTestPoint("cpu", models2.Tags{}, models2.Fields{"value": 1.0}, time.Unix(0, 1257894000000000000))
	for i, example := range examples {
		pts, err := models2.ParsePoints([]byte(example))
		if err != nil {
...
func TestParsePointUnescape(t *testing.T) {
	...
	test(t, `foo\,bar value=1i`,
		NewTestPoint(
			...
			models2.NewTags(map[string]string{}),
			models2.Fields{
				"value": int64(1),
			},
			time.Unix(0, 0)))
...
func TestParsePointWithTags(t *testing.T) {
	test(t,
		...
		NewTestPoint("cpu",
			models2.NewTags(map[string]string{"host": "serverA", "region": "us-east"}),
			models2.Fields{"value": 1.0}, time.Unix(1, 0)))
}
...
func TestParsePointWithDuplicateTags(t *testing.T) {
	for i, tt := range []struct {
		...
		_, err := models2.ParsePointsString(tt.line)
...
func TestParsePoint_TrailingSlash(t *testing.T) {
	_, err := models2.ParsePointsString(`a v=1 0\`)
	if err == nil {
...
func TestParsePointsUnbalancedQuotedTags(t *testing.T) {
	pts, err := models2.ParsePointsString("baz,mytag=\"a x=1 1441103862125\nbaz,mytag=a z=1 1441103862126")
	if err != nil {
		t.Fatalf("ParsePoints failed: %v", err)
	}
	...
	exp := models2.MustNewPoint("baz", models2.NewTags(map[string]string{"mytag": `"a`}),
		models2.Fields{"x": float64(1)}, time.Unix(0, 1441103862125))
	...
	exp = models2.MustNewPoint("baz", models2.NewTags(map[string]string{"mytag": `a`}),
		models2.Fields{"z": float64(1)}, time.Unix(0, 1441103862126))
...
func TestParsePointMaxTimestamp(t *testing.T) {
	test(t, fmt.Sprintf(`cpu value=1 %d`, models2.MaxNanoTime),
		NewTestPoint(
			"cpu",
			models2.NewTags(map[string]string{}),
			models2.Fields{
			...
			time.Unix(0, models2.MaxNanoTime)),
	)
}
...
func TestParsePointMinTimestamp(t *testing.T) {
	test(t, `cpu value=1 -9223372036854775806`,
		NewTestPoint(
			...
			time.Unix(0, models2.MinNanoTime)),
	)
}
...
func TestParsePointNaN(t *testing.T) {
	_, err := models2.ParsePointsString("cpu value=NaN 1000000000")
	if err == nil {
	...
	_, err = models2.ParsePointsString("cpu value=NaN")
	if err == nil {
		t.Fatalf("ParsePoints expected error, got nil")
	}
}
...
func TestNewPointLargeNumberOfTags(t *testing.T) {
	...
	pt, err := models2.ParsePointsString(fmt.Sprintf("cpu%s value=1", tags))
	if err != nil {
...
func TestParsePointIntsFloats(t *testing.T) {
	pts, err := models2.ParsePoints([]byte(`cpu,host=serverA,region=us-east int=10i,float=11.0,float2=12.1 1000000000`))
	if err != nil {
...
func TestParsePointKeyUnsorted(t *testing.T) {
	pts, err := models2.ParsePoints([]byte("cpu,last=1,first=2 value=1i"))
	if err != nil {
		t.Fatalf(`ParsePoints() failed. got %s`, err)
	}
...
func TestParsePointToString(t *testing.T) {
	...
	pts, err := models2.ParsePoints([]byte(line))
	if err != nil {
		t.Fatalf(`ParsePoints() failed. got %s`, err)
	}
	if exp := 1; len(pts) != exp {
	...
	pt = models2.MustNewPoint("cpu", models2.NewTags(map[string]string{"host": "serverA", "region": "us-east"}),
		models2.Fields{"int": 10, "float": float64(11.0), "float2": float64(12.123), "bool": false, "str": "string val"},
		time.Unix(1, 0))
...
func TestParsePointsWithPrecision(t *testing.T) {
	...
	for _, test := range tests {
		pts, err := models2.ParsePointsWithPrecision([]byte(test.line), time.Now().UTC(), test.precision)
		if err != nil {
...
func TestParsePointsWithPrecisionNoTime(t *testing.T) {
	...
	for _, test := range tests {
		pts, err := models2.ParsePointsWithPrecision([]byte(line), tm, test.precision)
		if err != nil {
			t.Fatalf(`%s: ParsePoints() failed. got %s`, test.name, err)
		}
		if exp := 1; len(pts) != exp {
...
cpu,host=serverA,region=us-east value=1.0 946730096789012345`,
	...
	for _, test := range tests {
		pts, err := models2.ParsePointsWithPrecision([]byte(test.batch), time.Now().UTC(), "")
		if err != nil {
			t.Fatalf(`%s: ParsePoints() failed. got %s`, test.name, err)
		}
...
func TestNewPointEscaped(t *testing.T) {
	// commas
	pt := models2.MustNewPoint("cpu,main", models2.NewTags(map[string]string{"tag,bar": "value"}), models2.Fields{"name,bar": 1.0}, time.Unix(0, 0))
	...
	// equals
	pt = models2.MustNewPoint("cpu=main", models2.NewTags(map[string]string{"tag=bar": "value=foo"}), models2.Fields{"name=bar": 1.0}, time.Unix(0, 0))
...
func TestNewPointWithoutField(t *testing.T) {
	_, err := models2.NewPoint("cpu", models2.NewTags(map[string]string{"tag": "bar"}), models2.Fields{}, time.Unix(0, 0))
	if err == nil {
...
func TestNewPointUnhandledType(t *testing.T) {
	// nil value
	pt := models2.MustNewPoint("cpu", nil, models2.Fields{"value": nil}, time.Unix(0, 0))
	...
	pt = models2.MustNewPoint("cpu", nil, models2.Fields{"value": now}, time.Unix(0, 0))
...
func TestMakeKeyEscaped(t *testing.T) {
	if exp, got := `cpu\ load`, models2.MakeKey([]byte(`cpu\ load`), models2.NewTags(map[string]string{})); string(got) != exp {
...
func TestPrecisionString(t *testing.T) {
	...
	for _, test := range tests {
		pt := models2.MustNewPoint("cpu", nil, tags, tm)
...
func TestParsePointsStringWithExtraBuffer(t *testing.T) {
	...
	points, err := models2.ParsePointsString(buf.String())
	if err != nil {
...
func TestParsePointsQuotesInFieldKey(t *testing.T) {
	buf := `cpu "a=1
cpu value=2 1`
	points, err := models2.ParsePointsString(buf)
	if err != nil {
		t.Fatalf("failed to write points: %s", err.Error())
	}
	...
	buf = `cpu "\, '= "\ v=1.0`
	_, err = models2.ParsePointsString(buf)
	if err == nil {
...
func TestNewPointsWithBytesWithCorruptData(t *testing.T) {
	...
	p, err := models2.NewPointFromBytes(corrupted)
	if p != nil || err == nil {
...
func TestNewPointsWithShortBuffer(t *testing.T) {
	_, err := models2.NewPointFromBytes([]byte{0, 0, 0, 3, 4})
...
func TestNewPointsRejectsEmptyFieldNames(t *testing.T) {
	if _, err := models2.NewPoint("foo", nil, models2.Fields{"": 1}, time.Now()); err == nil {
...
func TestNewPointsRejectsMaxKey(t *testing.T) {
	...
	for i := 0; i < models2.MaxKeyLength-len("value")-4; i++ {
		key += "a"
	}

	// Test max key len
	if _, err := models2.NewPoint(key, nil, models2.Fields{"value": 1, "ok": 2.0}, time.Now()); err != nil {
		t.Fatalf("new point with max key. got: %v, expected: nil", err)
	}

	if _, err := models2.ParsePointsString(fmt.Sprintf("%v value=1,ok=2.0", key)); err != nil {
...
func TestPoint_FieldIterator_Simple(t *testing.T) {
	...
	p, err := models2.ParsePoints([]byte(`m v=42i,f=42 36`))
	if err != nil {
		t.Fatal(err)
	}
...
func toFields(fi models2.FieldIterator) models2.Fields {
	m := make(models2.Fields)
	for fi.Next() {
		var v interface{}
		var err error
		switch fi.Type() {
		case models2.Float:
			v, err = fi.FloatValue()
		case models2.Integer:
			v, err = fi.IntegerValue()
		case models2.Unsigned:
			v, err = fi.UnsignedValue()
		case models2.String:
			v = fi.StringValue()
		case models2.Boolean:
...
func TestPoint_FieldIterator_FieldMap(t *testing.T) {
	...
	points, err := models2.ParsePointsString(`
m v=42
m v=42i
m v="string"
m v=true
...
func TestEscapeStringField(t *testing.T) {
	...
	for _, c := range cases {
		...
		got := models2.EscapeStringField(c.in)
		if got != c.expOut {
...
func TestParseKeyBytes(t *testing.T) {
	...
	for _, testCase := range testCases {
		t.Run(testCase.input, func(t *testing.T) {
			name, tags := models2.ParseKeyBytes([]byte(testCase.input))
			...
			if !tags.Equal(models2.NewTags(testCase.expectedTags)) {
				t.Errorf("%s produced tags %s but expected %s", testCase.input, tags.String(), models2.NewTags(testCase.expectedTags).String())
			}
		})
	}
}
...
func TestParseName(t *testing.T) {
	...
	for _, testCase := range testCases {
		t.Run(testCase.input, func(t *testing.T) {
			name := models2.ParseName([]byte(testCase.input))
			if !bytes.Equal([]byte(testCase.expectedName), name) {
				t.Errorf("%s produced measurement %s but expected %s", testCase.input, string(name), testCase.expectedName)
			}
		})
...
func BenchmarkEscapeStringField_Plain(b *testing.B) {
	s := "nothing special"
	for i := 0; i < b.N; i++ {
		sink = models2.EscapeStringField(s)
	}
}
...
func BenchmarkEscapeString_QuotesAndBackslashes(b *testing.B) {
	...
	for i := 0; i < b.N; i++ {
		sink = [...]string{models2.EscapeStringField(s1), models2.EscapeStringField(s2)}
	}
}
...
func BenchmarkEscapeMeasurement(b *testing.B) {
	...
	for _, bm := range benchmarks {
		b.Run(string(bm.m), func(b *testing.B) {
			b.ReportAllocs()
			for i := 0; i < b.N; i++ {
				models2.EscapeMeasurement(bm.m)
			}
		})
	}
}

func makeTags(key, val string, n int) models2.Tags {
	tags := make(models2.Tags, n)
	for i := range tags {
